...
mod tests {
    ...
    fn test_tree_node_creation() {
        ...
        assert_eq!(node.metadata.get("test_key"), Some(&"test_value".to_string()));
    }
    ...
    fn test_content_extractor() {
        ...
        let extractor_with_format = ContentExtractor::with_format(
            "title", "items", "Section: {}"
        );
        assert_eq!(extractor_with_format.content_property, "title");
        assert_eq!(extractor_with_format.children_property, "items");
        assert_eq!(extractor_with_format.format_template, Some("Section: {}".to_string()));
    }
    ...
    fn test_get_node_type_name() {
        ...
        let session_container = ElementNode::SessionContainer(SessionContainer::new(
            vec![], vec![], Parameters::default(), TokenSequence::new()
        ));
    ...
    fn test_synthetic_basic_ast_nodes() {
        ...
        for (node, expected_type, expected_icon) in test_cases {
            ...
            assert!(result.is_ok(), "Failed to convert {} to tree notation", expected_type);
            
            let output = result.unwrap();
            assert!(output.contains(expected_icon), 
                   "Output for {} should contain icon {}", expected_type, expected_icon);
        }
    }
    ...
    fn test_basic_inline_elements() {
        ...
        for (node, expected_type, expected_icon) in simple_test_cases {
            // Test node type detection
            assert_eq!(get_node_type_name(&node), expected_type);
    ...
    fn test_comprehensive_icon_coverage() {
        ...
        println!("✅ All {} icons from GitHub issue #46 are correctly configured", 
