...
fn test_reference_classification() {
    ...
    assert_eq!(
        classifier.classify("user@domain.com"),
        SimpleReferenceType::Url
    );

    // Test citation classification
    assert_eq!(
        classifier.classify("@smith2023"),
        SimpleReferenceType::Citation
    );
    assert_eq!(
        classifier.classify("@smith2023, p. 45"),
        SimpleReferenceType::Citation
    );
    ...
    assert_eq!(
        classifier.classify("TK-someword"),
    ...
    assert_eq!(
        classifier.classify("../dir/file.txt"),
        SimpleReferenceType::File
    );
    assert_eq!(
        classifier.classify("/absolute/path"),
        SimpleReferenceType::File
    );
