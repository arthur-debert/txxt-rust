...
fn test_two_paragraphs_missing_second() {
    ...
    let document = full_pipeline(&corpus.source_text, Some("01-two-paragraphs.txxt".to_string()))
    ...
    let expected_texts = vec![
        "This is the first paragraph of a simple document. It contains plain text without any special formatting or structure.",
...
fn extract_paragraph_text(paragraph: &txxt::ast::elements::paragraph::block::ParagraphBlock) -> String {
    paragraph.content
        .iter()
...
fn test_current_behavior_one_paragraph() {
    ...
    let document = full_pipeline(&corpus.source_text, Some("test".to_string()))
        .expect("Failed to parse");
    
    // This currently passes but shouldn't - it demonstrates the bug
    let paragraph_count = document.content.content.iter()
    ...
    println!("Current implementation finds {} paragraph(s)", paragraph_count);
    assert!(
        paragraph_count >= 1,
        "Should find at least one paragraph"
    );
