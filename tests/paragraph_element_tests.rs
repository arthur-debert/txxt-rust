...
use corpora::{TxxtCorpora, ProcessingStage};
...
fn assert_paragraph_contains_text(paragraph: &txxt::ast::elements::paragraph::ParagraphBlock, expected: &str) {
    assert_eq!(paragraph.content.len(), 1, "Paragraph should have exactly one content element");
    
    if let txxt::ast::elements::inlines::TextTransform::Identity(text_span) = &paragraph.content[0] {
        let text = text_span.tokens.text();
        assert!(text.contains(expected), "Text '{}' should contain '{}'", text, expected);
    } else {
...
fn assert_paragraph_structure(paragraph: &txxt::ast::elements::paragraph::ParagraphBlock) {
    assert_eq!(paragraph.annotations.len(), 0, "Simple paragraphs should have no annotations");
    assert_eq!(paragraph.parameters.map.len(), 0, "Simple paragraphs should have no parameters");
}
...
fn test_corpora_simple_paragraph() {
    ...
    assert!(corpus.source_text.contains("paragraph"), "Should be about paragraphs");
...
fn test_corpora_multiline_paragraph() {
    ...
    assert!(!corpus.source_text.is_empty(), "Corpus should have content");
    assert!(corpus.source_text.contains("line"), "Should be about multiline content");

    let tokens = txxt::lexer::tokenize(&corpus.source_text);
    let result = parse_paragraph(&tokens);
    assert!(result.is_ok(), "Multiline paragraph should parse successfully");
...
fn test_corpora_consistent_indent_paragraph() {
    ...
    let tokens = txxt::lexer::tokenize(&corpus.source_text);
    let result = parse_paragraph(&tokens);
    assert!(result.is_ok(), "Consistent indent paragraph should parse successfully");
...
fn test_ensemble_single_paragraph() {
    ...
    assert!(!corpus.source_text.is_empty(), "Document should have content");
    assert!(corpus.source_text.contains("paragraph"), "Should contain paragraph content");

    let tokens = txxt::lexer::tokenize(&corpus.source_text);
    let result = parse_paragraph(&tokens);
    assert!(result.is_ok(), "Single paragraph document should parse successfully");
...
fn test_ensemble_multiple_paragraphs() {
    ...
    assert!(!corpus.source_text.is_empty(), "Document should have content");
    assert!(corpus.source_text.contains("first paragraph"), "Should contain first paragraph");
    assert!(corpus.source_text.contains("second paragraph"), "Should contain second paragraph");
...
fn test_all_paragraph_corpora() {
    ...
    assert!(!paragraph_corpora.is_empty(), "Should have paragraph corpora");
